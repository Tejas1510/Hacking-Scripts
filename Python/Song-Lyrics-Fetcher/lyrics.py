...
genius = lg.Genius('Insert_Client_API_Token_Here', skip_non_songs=True, excluded_terms=["(Remix)", "(Live)"], remove_section_headers=True)
...
filename = '-'.join((title + ' ' + artist).split()) + '-lyrics.txt'  # filename for the file is the name of song
...
if os.path.exists(filename):  # If the file already exists
    ...
    file = open(filename, 'w')  # Opening the file with filename in writing mode
    try:
        ...
        print('Error finding the lyrics of the song "' + title + '" by', artist)  # Print an error if the lyrics are not found
